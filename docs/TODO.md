...
- 🚧 **0단계 준비 (08/22~08/26)** — 영업이사 30분 인터뷰(타깃 10개사·첫 멘트·자주 받는 질문·업무 용어) → 질문지 v1 문구 확정, 소개서 1장 PDF 초안, `?ref=` 코드·EDITOR 계정·`SALES_NOTIFY_EMAIL` 확정, 3단계 발송 담당자 지정 — 인터뷰(0-2)·문구 확정(0-3)·소개서(0-4)·계정 발급(0-5)·담당자 지정(0-7)은 사람이 하는 일이라 남아 있음. ✅ `AxCheckResponse` 수동 `migration.sql` 작성 완료(0-6, 2026-08-25)
