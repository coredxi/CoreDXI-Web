...
- 🚧 **1단계 영업 가동 (08/27~09/05)** — 코드 구현 완료(2026-08-25): `catalog.ts`+`summarize.ts`(등급 규칙·Q3 매핑·Q5 분기, Vitest 13개) + `src/actions/ax-check.ts`(검증·rate limit·저장·옵트인 구독·메일 2통, Vitest 25개) + `/ax-check`(8문항 전부 선택지, 3분, `?ref=` 보존) + `/ax-check/result/[token]` + `/admin/leads`(HOT/WARM/COLD·상태·메모·CSV·삭제) + GA4 `ax_check_submit`(`source`=ref) 이벤트·퍼널 패널 병기 + `/privacy`·`CONTENT_GUIDE.md` 17번. 질문지 문구는 여전히 v1-draft(0-3 인터뷰 결과 대기, `catalog.ts` 상단 주석 참고). ⬜ 남은 것: 영업이사 실기기 테스트, **첫 링크 발송**. `prisma migrate deploy`·Playwright E2E는 2026-08-26~27 완료.
  ...
  - ✅ **C단계 구현, 계획보다 앞당겨 완료(2026-09-02~09/03)** — `feat/ax-check-auto-followup` 브랜치를 따로 만들지 않고 **main에 직접 커밋 18건**으로 구현·배포됨(계획 5-C 항목 순서와 다름, 계획서 브랜치 지침에서 벗어난 점 기록). `followupStatus` 등 7필드 마이그레이션 프로덕션 반영 완료(`prisma migrate status`로 확인), 영업일 계산(`business-days.ts`)·T0/T1 발송 파이프라인(`followup.ts`)·Vercel Cron(`/api/cron/ax-check-followup`, 매일 09:30 KST)·`/admin/leads` 팔로업 패널(보류·해제·지금 보내기·본문 수정·이력)·결과 화면/`/privacy`/`CONTENT_GUIDE.md` 문구 전부 반영, CI(lint/tsc/vitest) 전 커밋 green. C-8 프로덕션 검증: `ref=test-c8` 테스트 리드 제출→영업이사 알림 수신 확인(09/02 22:42), 등급×업종 9건 육안 검수 완료(09/03) 중 답변 인용 조사 병기("을(를)") 어색함 발견해 `objectParticle()` 헬퍼로 수정(커밋 `acb9561`). **남은 C-8 항목**: T1 실제 수신 확인(D+2 영업일=09/04 09:30 KST 예정, 아직 미도래), 보류(HELD) 케이스 스킵 확인, Vercel 프로덕션 env(`CRON_SECRET`·`AX_CHECK_FOLLOWUP_ENABLED` 등) 실제 설정 여부(사용자 확인 필요 — 이 세션에 연결된 Vercel 계정이 실제 배포 계정과 달라 대시보드 조회 불가). 상세 체크리스트는 액션플랜·설계문서에 반영(2026-09-03)
