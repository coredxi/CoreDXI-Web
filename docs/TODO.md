...
> 최종 업데이트: 2026-09-03
> 코드베이스 분석 기반 — 실제 구현 상태를 반영합니다. (2026-06-30 → 2026-07-07 종합 고도화 세션, 2026-07-19 rate limiting·다크모드 세션, 2026-08-05 CSP Enforcing 전환, 2026-08-06 소셜 메타태그 강화 착수, 2026-08-07 배포·전채널 검증까지 완료, 2026-08-08 뉴스레터 구독 구현, 2026-08-14 블로그 콘텐츠 전량 삭제·재건 전략 수립 + 전환 퍼널 대시보드 2단계·블로그 하단 CTA 구현, 2026-08-22 영업 지원 트랙(Phase 1.5) 설계·액션플랜 수립, 2026-08-30 전환 퍼널 대시보드 실측 완료 및 Phase 1 공식 종료, 2026-09-02 영업채널 자동 팔로업 전환 결정·구현, 2026-09-03 프로덕션 검증·소개서 v3 확정)
...
- 🚧 **0단계 준비 (08/22~08/26, 실제 08/31까지 연장)** — ✅ 영업이사 서면 인터뷰 회신 수령·정리 완료(0-2, 2026-08-31): `docs/superpowers/specs/2026-08-31-sales-director-interview-response.md`(타깃 6개사·첫 멘트·자주 받는 질문·업무 용어 전부 확보). ✅ 질문지 v1 문구 확정(0-3) — `catalog.ts` 코드 반영 완료(2026-09-01, 커밋 `e7d045a`, CATALOG_VERSION v2). tsc 통과, vitest/eslint는 로컬(Windows pnpm) 또는 CI에서 최종 확인 필요. ⬜ 담당자 지정(0-7)은 아직 사람이 정할 일로 남아 있음. ✅ **소개서(0-4)는 2026-08-30 1장 → 4~5p 슬라이드 PDF로 확장 결정, 사용자가 다른 세션 v1_draft(일반형 문구, 인터뷰 미반영)를 최종 채택(2026-09-01) — AI 비서가 인터뷰 반영해 새로 만든 v2_draft는 미채택. 영업이사 2차 검토(S2)+사용자 키포인트 4건 반영해 v3 제작 완료(2026-09-01, `20260901_…_v3.pptx/.pdf` — 페인포인트 2종 추가·4단계 정합 확인·4p 이모지 강화·5p '왜 CoreDXI' 20년 경력 중심 재편). **v3 최종 확정(2026-09-03) → S4 게시 완료**: `/solutions` AX 컨설팅 카드에 소개서 다운로드 버튼 추가(`public/docs/coredxi-ax-consulting-brochure.pdf`, `SolutionsContent.brochureLabel`·`brochureUrl` CMS 필드), 브랜치 `feat/solutions-brochure-download` PR로 게시: `docs/superpowers/plans/2026-08-30-ax-consulting-brochure-action-plan.md`, `docs/superpowers/plans/2026-09-03-solutions-brochure-download-claude-code-prompt.md`** ✅ `AxCheckResponse` 수동 `migration.sql` 작성 완료(0-6, 2026-08-25). ✅ `?ref=`코드·EDITOR 계정·`SALES_NOTIFY_EMAIL` 확정 완료(0-5, 2026-08-26) — ref=`sales-kim`, `scripts/create-sales-editor-account.ts`로 `obaamg1017@coredxi.com` EDITOR 계정 생성, `SALES_NOTIFY_EMAIL=devcoredxi00@coredxi.com`을 `.env.local`·Vercel Production/Preview에 등록
- 🚧 **1단계 영업 가동 (08/27~09/05)** — 코드 구현 완료(2026-08-25): `catalog.ts`+`summarize.ts`(등급 규칙·Q3 매핑·Q5 분기, Vitest 13개) + `src/actions/ax-check.ts`(검증·rate limit·저장·옵트인 구독·메일 2통, Vitest 25개) + `/ax-check`(8문항 전부 선택지, 3분, `?ref=` 보존) + `/ax-check/result/[token]` + `/admin/leads`(HOT/WARM/COLD·상태·메모·CSV·삭제) + GA4 `ax_check_submit`(`source`=ref) 이벤트·퍼널 패널 병기 + `/privacy`·`CONTENT_GUIDE.md` 17번. 질문지 문구는 여전히 v1-draft(0-3 인터뷰 결과 대기, `catalog.ts` 상단 주석 참고). ⬜ 남은 것: 영업이사 실기기 테스트, **첫 링크 발송** — 최초 목표(08/27~09/05)에서 지연 중. 09/02 1차 실기기 테스트에서 수동 발송 워크플로우 문제를 확인해 자동 팔로업 전환을 결정(아래 항목), 전환 반영분(C단계) 위에서 재테스트 후 발송 예정. `prisma migrate deploy`·Playwright E2E는 2026-08-26~27 완료.
  ...
  - 🔄 **2026-09-02 방향 재결정 — 자동 팔로업 전환**: 실기기 테스트 결과 영업이사 1인·외근 체제에서 "담당 이사 검토 후 수동 발송"은 지켜지지 않는다고 판단. 결과 화면 문구를 "영업일 기준 2~3일 내"로 1차 수정(커밋 `783b55b`)한 뒤, 8/30의 수동 발송 워크플로우를 철회하고 **시스템이 D+2 영업일에 상세 진단 메일을 자동 발송(발송 전 보류·수정 창 제공) + 영업이사는 HOT 리드 통화만** 담당하는 구조로 전환 결정. 첫 메일 콘텐츠는 별도 검토. 상세: `docs/superpowers/plans/2026-09-02-sales-channel-auto-followup-action-plan.md` (A 결정 09/02~04 → B 설계 ~09/05 → C 구현 09/08~12 → D 운영 전환 ~09/16 → E 2차 ~09/26)
  - 🔧 **2026-09-05 영업 알림 메일 수신 구조 변경(첫 발송 준비)** — 사용자 결정: 새 리드 알림 메일의 수신(to)=영업이사, 참조(cc)=기술이사. `sendResendEmail`에 `cc` 지원 추가(`src/lib/resend.ts`), `src/actions/ax-check.ts`가 `SALES_NOTIFY_CC_EMAIL`(선택, 수신과 같으면 무시)을 cc로 넣도록 수정, Vitest 2건 추가, `.env.example`·`CONTENT_GUIDE.md` 갱신. **후속(사용자)**: Vercel Production/Preview에 `SALES_NOTIFY_EMAIL=obaamg1017@coredxi.com`(변경), `SALES_NOTIFY_CC_EMAIL=devcoredxi00@coredxi.com`(신규) 등록 후 재배포 → 테스트 리드 1건으로 수신·참조 확인. 이 세션(원격)에서는 tsc만 통과 확인, vitest·lint는 로컬/CI에서 확인 필요. 같은 날 영업이사 첫 발송 지원 산출물 제작: `docs/superpowers/assets/sales-followup/20260905_안내문_사내_AX체크영업이사사용매뉴얼_v2.pdf/.docx`(모바일 사용 매뉴얼) + `20260905_템플릿_사내_AX체크첫링크카톡메시지_v1.md`(카톡 메시지 4종). 1-10 실기기 테스트는 사용자가 완료 확인(2026-09-05), 1-11 첫 링크 발송만 남음
  - ✅ **C단계 구현, 계획보다 앞당겨 완료(2026-09-02~09/03)** — `feat/ax-check-auto-followup` 브랜치를 따로 만들지 않고 **main에 직접 커밋 18건**으로 구현·배포됨(계획 5-C 항목 순서와 다름, 계획서 브랜치 지침에서 벗어난 점 기록). `followupStatus` 등 7필드 마이그레이션 프로덕션 반영 완료(`prisma migrate status`로 확인), 영업일 계산(`business-days.ts`)·T0/T1 발송 파이프라인(`followup.ts`)·Vercel Cron(`/api/cron/ax-check-followup`, 매일 09:30 KST)·`/admin/leads` 팔로업 패널(보류·해제·지금 보내기·본문 수정·이력)·결과 화면/`/privacy`/`CONTENT_GUIDE.md` 문구 전부 반영, CI(lint/tsc/vitest) 전 커밋 green. **C-8 프로덕션 검증도 완료** — Notion 액션 DB(fifty-ledger)에 다른 세션에서 이미 사용자 확인을 거쳐 완료 처리된 기록 확인(T0/T1 수신, 관리자 보류/해제/지금 보내기, 크론 인증, Vercel 프로덕션 env 등록 전부 통과). 이번 세션에서 직접 확인한 것: `ref=test-c8` 테스트 리드 제출→영업이사 알림 수신(09/02 22:42, Gmail), 등급×업종 9건 육안 검수 신규 진행(09/03) 중 답변 인용 조사 병기("을(를)") 어색함 발견해 `objectParticle()` 헬퍼로 수정(커밋 `acb9561`). **유일하게 남은 것**: Playwright E2E(`ax-check.spec.ts`) 재실행 — 프로덕션 DB 직결이라 세션에서 임의 실행 보류. 체크리스트 상세는 액션플랜·설계문서에 반영(2026-09-03, Notion과 재동기화)
- 🚧 **2단계 보강 (09/08~09/26) — 2026-09-02 재편**: 자동 팔로업 시퀀스(T0 즉시 요약 메일·T1 D+2 상세 진단 자동 발송·관리자 보류/즉시발송·영업일 계산·Vercel Cron) **구현 완료(위 항목 참고, 계획보다 이르게 09/02~03 마무리)**. 남은 것: 완료율 점검(20% 미만이면 문항 8→6), `/solutions` 단일 오퍼 재편(+`/admin/solutions` 폼·가이드 갱신), `/about` 레퍼런스 중심 축약, 블로그 1편 — 전부 미착수. LLM 상세 진단서는 자동 발송 파이프라인 위의 후속 개선(응답 20건 이상)으로 이동
