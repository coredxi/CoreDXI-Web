# Claude Code 작업 프롬프트 — AX 체크 자동 팔로업 (2026-09-02)

> 사용법: 아래 프롬프트 전문을 복사해 `C:\MyProjects\CoreDXI-Web`에서 실행한 Claude Code 세션에 붙여넣으세요.
> 선행: A-2 메일 초안 선택(`docs/superpowers/plans/2026-09-02-ax-check-followup-email-drafts.md`)이 끝나 있어야 `FOLLOWUP_COPY` 문구를 확정할 수 있습니다. 아직이면 "추천 조합(T0 안 2, T1 안 1)"으로 진행하고 상수만 나중에 바꿉니다.
...
- 메일 문구: docs/superpowers/plans/2026-09-02-ax-check-followup-email-drafts.md
  (선택된 안이 표시되어 있지 않으면 T0=안 2, T1=안 1 조합으로 진행)
...
리드가 들어오면 사람 없이도 시스템이 (T0) 제출 즉시 결과 요약 메일을, (T1) D+2 영업일 09:30 KST에
상세 진단 메일을 자동 발송한다. 영업이사는 메일을 쓰지 않고 HOT 리드 통화만 한다.
관리자는 발송 전에 보류·수정·즉시 발송할 수 있다. 8/30의 "초안 + 수동 발송"은 이 작업으로 대체된다.

## 착수 전 (프로젝트 지침 5-1)

1. docs/PRD.md·docs/TODO.md·CLAUDE.md 2절에 이번 변경이 이미 반영되어 있는지 확인하고, 빠진 부분이
   있으면 먼저 보강한다(2026-09-02 AI 비서 세션이 1차 반영함).
...
### 3. 메일 문구·초안 (설계 8절)
- src/lib/ax-check/catalog.ts: INTRO_COPY 옆에 FOLLOWUP_COPY 상수(T0 제목·본문 조각, T1 제목·여는 말·
  맺는 말, 공통 안내 문구). 홍보팀이 수정할 수 있게 [홍보팀] 주석.
- src/lib/ax-check/email-draft.ts:
  - buildCustomerEmailDraft(answers, summary, contact, opts?: { mode?: "manual" | "auto" }) —
    "auto"는 [[통화에서 말씀 주신 ___]] 슬롯을 제거하고 FOLLOWUP_COPY의 T1 여는 말·맺는 말 사용.
    "manual"(기본)은 기존 출력과 동일해야 한다(기존 테스트 유지).
  - 신규 buildT0Email(summary, contact, links: { resultUrl, brochureUrl?: string }) → { subject, body }.
    brochureUrl이 없으면 소개서 줄을 통째로 생략.
- email-draft.test.ts 갱신·추가.

### 4. 발송 파이프라인 (설계 6절)
- src/lib/ax-check/followup.ts: isFollowupEnabled / sendFollowupEmail(id, { force }) /
  processDueFollowups({ now, limit }). 선점은 updateMany(count===1)로, 발신 replyTo는
  process.env.SALES_REPLY_TO ?? SALES_SIGNATURE.email, 실패 시 FAILED+attempts+Sentry.captureException.
...
- src/actions/ax-check.ts submitAxCheck:
  ...
  - T0 발송(sendResendEmail) → 성공 시 t0SentAt. 실패는 로그·Sentry, 제출 성공은 유지.
...
### 8. 문구·문서 (설계 10절)
- src/components/ax-check/AxCheckPriorityCards.tsx 안내 문구 →
  "정리된 상세 진단서를 영업일 기준 2~3일 내 메일로 보내드립니다. 우선 과제가 뚜렷한 경우 담당 이사가 직접 연락드립니다."
- AxCheckForm 제출 완료 안내에 "결과 요약 메일을 보내드렸습니다" 추가.
- src/app/privacy/page.tsx AX 체크 조항에 결과 안내 메일 자동 발송 명시.
- .env.example: CRON_SECRET / SALES_REPLY_TO(선택) / AX_CHECK_BROCHURE_URL(선택) / AX_CHECK_FOLLOWUP_ENABLED(선택, 기본 true) 주석 포함.
- CONTENT_GUIDE.md 17번: FOLLOWUP_COPY 위치, 보류·수정·지금 보내기 방법, 공휴일 상수 갱신, 킬 스위치.
- docs/superpowers/specs/2026-08-30-ax-check-experience-upgrade-design.md 5절 상단에
  "2026-09-02 자동 발송 설계로 대체됨" 한 줄이 이미 있는지 확인.
...
- PR 본문에 설계 문서 링크 + DoD 체크리스트(설계 14절)를 붙이고, 사용자가 할 일을 명시:
  ① Vercel env CRON_SECRET(랜덤 32자+)·SALES_REPLY_TO 등록 ② prisma migrate deploy ③ SALES_SIGNATURE 실제 값
  ④ C-8 프로덕션 검증(테스트 ref 제출 → T0 → 크론 수동 호출 → T1 → 보류 1건).

## 하지 말 것
- prisma migrate dev, 기존 리드에 소급 자동 발송(SKIPPED 기본값 유지), HTML 메일 도입(텍스트 유지),
  새 외부 라이브러리(날짜·크론), 새 색상 토큰, any 타입, default export.
