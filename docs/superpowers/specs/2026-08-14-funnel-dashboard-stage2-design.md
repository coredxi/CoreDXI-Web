...
- [x] GA4 관리 콘솔 → 관리 → 맞춤 정의에서 `percent`/`cta_location`/`source` 커스텀 디멘션 등록 여부 확인. **2026-08-16**: 미등록 상태였음을 확인 후 사용자가 3개 모두 신규 등록(이벤트 범위) — 등록 시점 이후 이벤트부터만 파라미터 단위 조회 가능(소급 없음). 이번 2단계는 3번 결정사항대로 이벤트 총합만 사용 유지, 스크롤 세분화 추가는 범위 밖으로 남김(스코프 확장 금지 원칙)
